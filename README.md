...
- Mantle can fail a generation **mid-stream**, over an otherwise-`200 OK` connection — it sends a `data: {"error": {...}}` chunk (OpenAI contract) or an `event: error` (Anthropic contract) instead of the next content chunk, with no non-2xx status to catch it by. Both streaming translators detect this and immediately emit a proper error event to the client instead of silently finishing as if the completion had succeeded (this was the root cause of a report where Gemma 4 via Claude Code produced no output and no visible error — Mantle's own engine returned `Task submission failed... Generation failed`, and the untranslated error chunk was dropped because it had no `choices`). The underlying generation failure itself is on Mantle's side, not something this proxy can fix — if you hit it, check whether a smaller/simpler request (e.g. fewer tool definitions) succeeds, to narrow down whether it's payload-size/complexity related.
- The console logs a small sample of the raw (pre-translation) SSE chunks Mantle sends for any cross-contract streaming request (`app/orchestrator.py`, `_log_raw_chunks`) — useful for diagnosing a translated stream that looks empty or wrong, since it lets you see whether Mantle sent nothing usable or sent something in a shape the translator doesn't recognize.
