...
- The proxy forwards the client's `anthropic-beta` header verbatim to Mantle's Anthropic contract (previously it was silently dropped) — some Anthropic-API fields are only recognized server-side when the matching beta value is present.
