...
- Mantle's native Anthropic Messages API endpoint doesn't support structured outputs: a request carrying `output_config.format` is rejected with `400 output_config.format: Extra inputs are not permitted`. Claude Code sends this for some internal calls (e.g. session-title generation), which would otherwise fail every time. `mantle_client._strip_unsupported_anthropic_fields` drops just that sub-field before forwarding to the Anthropic contract (logging a warning when it does) — other `output_config` keys, like `effort`, are left untouched.
